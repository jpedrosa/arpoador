...
void listFiles(String dirPath, fn(name, type)) {
  var fd = MoreSys.openDir(dirPath);
  if (fd != -1) {
    try {
...
void recurseDir(String dirPath, fn(String name, int type,
    String dirPath)) {
...
void doRecurseDir(String dirPath, fn(String name, int type,
    String dirPath)) {
  try {
    listFiles(dirPath, (name, type) {
      if (name != ".." && name != ".") {
...
main() {
  p("yes");
  listFiles("/home/dewd/t_/jungle", (name, type) {
    p([name, type]);
  });
  p("-----");
  recurseDir("/home/dewd/t_", (name, type, dirPath) {
    p("${dirPath}/${name}");
  });
}
